            ...
            <div class="alert alert-info border-0 shadow-sm p-3">
                <div class="d-flex align-items-start">
                    ...
                    <div>
                        ...
                        <p class="mb-2 fs-13">Agar permohonan Anda dapat diproses oleh Dinas Perdagangan, pastikan
                            mengikuti langkah-langkah berikut:</p>
                        <ol class="mb-0 fs-13 ps-3">
                            <li>Klik tombol <strong>"Lihat / Cetak Draf"</strong> untuk memeriksa dan mencetak fisik
                                surat permohonan.</li>
                            <li>Bubuhkan tanda tangan asli Anda (Pemohon) pada lembar surat permohonan fisik.</li>
                            <li>Pindai (scan) atau foto lembar surat bertandatangan tersebut dengan jelas (format PDF,
                                JPG, atau PNG).</li>
                            <li>Klik tombol <strong>"Unggah Surat"</strong> untuk mengunggah dokumen dan mengubah status
                                menjadi <strong>Lengkap</strong>.</li>
                        </ol>
                    </div>
                </div>
            </div>
                ...
                <div class="card-header bg-transparent border-bottom">
                    <div class="row align-items-center">
                        <div class="col-auto">
                            ...
                            <p class="text-muted mb-0 fs-13">Pantau kelengkapan dokumen dan status verifikasi permohonan
                                tempat pasar.</p>
                        </div>
                ...
                <div class="card-body p-0">
                    @if ($permohonans->isEmpty())
                        <div class="text-center py-5">
                            <div
                                class="avatar-lg bg-light rounded-circle mx-auto d-flex align-items-center justify-content-center mb-3 text-muted">
                                <i class="iconoir-page-flip fs-36"></i>
                            </div>
                            <h5 class="fw-bold">Belum Ada Permohonan</h5>
                            <p class="text-muted fs-13 mb-3">Anda belum membuat pengajuan surat permohonan tempat pasar.
                            </p>
                            <a href="{{ route('pedagang.ajukan_permohonan.create') }}" class="btn btn-primary btn-sm"
                                wire:navigate>
                                    ...
                                    @foreach ($permohonans as $index => $p)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <small
                                                    class="text-muted">{{ \Carbon\Carbon::parse($p->created_at)->format('H:i') }}
                                                    WIB</small>
                                            </td>
                                            <td>
                                                <div class="fw-bold text-dark">{{ $p->pasar->nama_pasar ?? '-' }}</div>
                                                <div class="fs-13 text-muted">
                                                    {{ ucfirst($p->tipe_tempat) }} Nomor
                                                    <strong>{{ $p->nomor_tempat }}</strong>
                                                    @if ($p->luas)
                                                        ({{ $p->luas }} m²)
                                                    @endif
                                                </div>
                                            ...
                                            <td class="text-center">
                                                ...
                                                @else
                                                    <span
                                                        class="badge bg-secondary-subtle text-secondary px-2 py-1">{{ ucfirst($p->status) }}</span>
                                                @endif
                                            </td>
                                            <td class="text-center">
                                                @if ($p->dokumen_path)
                                                    <a href="{{ asset('storage/' . $p->dokumen_path) }}"
                                                ...
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu shadow">
                                                        <a class="dropdown-item" href="javascript:void(0)"
                                                            wire:click="previewSurat({{ $p->id }})">
                                                            <i class="iconoir-page-search me-2 text-primary"></i> Draf
                                                            Surat
                                                        </a>
                                                        @if ($p->status === 'draft')
                                                            <a class="dropdown-item"
                                                                href="{{ route('pedagang.unggah_permohonan.create', ['id' => $p->id]) }}"
                                                                wire:navigate>
                                                                <i class="iconoir-upload me-2 text-success"></i> Unggah
                                                                Surat
                                                            </a>
                                                        @endif
                                                        @if ($p->dokumen_path)
                                                            <a class="dropdown-item"
                                                                href="{{ asset('storage/' . $p->dokumen_path) }}"
                                                                target="_blank">
                                                                <i class="iconoir-open-new-window me-2 text-info"></i>
                                                                Berkas Terunggah
                                                            </a>
                                                        @endif
                                                    </div>
